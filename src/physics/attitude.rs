...
use crate::numerics::quaternion::{Quaternion, compute_quaternion_derivative};
...
pub fn calculate_torque(state: &State) -> na::Vector3<f64> {
    ...
    (3.0 * crate::constants::G * crate::constants::M_EARTH 
        / (2.0 * r_mag.powi(3))) 
        * z_body.cross(&(inertia * z_body))
}

pub fn angular_acceleration(state: &State, external_torque: Option<na::Vector3<f64>>) -> na::Vector3<f64> {
    let inertia = state.inertia_tensor;
